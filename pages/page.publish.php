...
<div class="grid-layout">
    <main class="overflow-x-hidden bg-[#252526]  md:p-8">

        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-white">Publish Store</h1>
                <p class="text-zinc-400 text-sm mt-1">Publish to <span
                        class="text-violet-400 font-medium"><?php echo htmlspecialchars($store_domain); ?></span></p>
            </div>
            <a href="<?php echo $admin_base; ?>"
                class="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-5 py-2.5 rounded-lg text-sm font-medium transition-colors">
                Back to Dashboard
            </a>
        </div>
        ...
        <?php if (isset($_GET['success'])): ?>
            <div
                class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-4 rounded-xl mb-8 flex items-center gap-3">
                <i class="bi bi-check-circle-fill text-xl"></i>
                <span class="text-sm font-medium">Store successfully
                    <?php echo htmlspecialchars($_GET['success']); ?>!</span>
            </div>
        <?php endif; ?>

        <!-- Publish Action -->
        <div class="bg-[#252526] border border-zinc-800 rounded-xl p-6 mb-8 relative overflow-hidden">
            <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                    <h3 class="text-xl font-bold text-white">Deploy Updates</h3>
                    <p class="text-zinc-400 text-sm mt-2 max-w-xl">Compile your products, theme, and configuration into
                        a high-performance static HTML site and publish it live.</p>
                </div>
                <form method="POST">
                    <input type="hidden" name="action" value="publish">
                    <button type="submit"
                        class="bg-violet-600 hover:bg-violet-500 text-white px-8 py-4 rounded-xl font-bold transition-colors shadow-lg shadow-violet-500/20 flex items-center gap-2">
                ...
                <?php else: ?>
                    <?php foreach ($deployments as $index => $dep): ?>
                        <div class="px-6 py-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="text-white font-medium font-mono text-sm">v_<?php echo htmlspecialchars($dep['version_hash']); ?></span>
                                        <?php if ($index === 0): ?>
                                            <span
                                                class="bg-emerald-500/20 text-emerald-400 text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded">Current
                                                Live</span>
                                        <?php endif; ?>
                                        <?php if ($dep['status'] === 'rollback'): ?>
                                            <span
                                                class="bg-amber-500/20 text-amber-400 text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded">Rollback</span>
                                        <?php endif; ?>
                                    </div>
                            ...
                            <?php if ($index !== 0): ?>
                                <form method="POST"
                                    onsubmit="return confirm('Are you sure you want to rollback to this version?');">
                                    <input type="hidden" name="action" value="rollback">
                                    <input type="hidden" name="deployment_id" value="<?php echo $dep['id']; ?>">
                                    <button type="submit"
                                        class="text-sm font-medium text-amber-400 hover:text-amber-300 bg-amber-400/10 hover:bg-amber-400/20 px-4 py-2 rounded-lg transition-colors border border-amber-400/20">
                                        <i class="bi bi-arrow-counterclockwise mr-1"></i> Rollback
                                    </button>
                                </form>
                            <?php else: ?>
                                <button disabled
                                    class="text-sm font-medium text-zinc-600 bg-zinc-800 px-4 py-2 rounded-lg cursor-not-allowed">
                                    Active
                                </button>
                            <?php endif; ?>
                        </div>
