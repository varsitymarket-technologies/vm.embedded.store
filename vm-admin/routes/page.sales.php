...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_sale') {
        ...
        $sql = "INSERT INTO sales (name, percentage, status) VALUES (?, ?, ?)";
        $db->query($sql, [$name, $percentage, $status]);
        ...
        $sql = "UPDATE sales SET name = ?, percentage = ?, status = ? WHERE id = ?";
        $db->query($sql, [$name, $percentage, $status, $id]);
...
?>
        ...
        <div class="flex flex-1 flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto overflow-x-hidden bg-gray-900 p-6">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-semibold">Sales Campaigns</h2>
                        ...
                        <table class="w-full text-left text-sm text-gray-400">
                            <thead class="bg-gray-700/50 text-xs uppercase text-gray-300">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3">Percentage Off</th>
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                            <div class="flex flex-col items-center justify-center">
                                                <i class="bi bi-tag text-4xl mb-2"></i>
                                                <p>No sales found. Click "Add Sale" to create one.</p>
                                            </div>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    ...
                                    <tr class="hover:bg-gray-700/30 transition-colors">
                                        <td class="px-6 py-4 font-medium text-white">#<?php echo $sale['id']; ?></td>
                                        <td class="px-6 py-4 font-medium text-white"><?php echo htmlspecialchars($sale['name']); ?></td>
                                        <td class="px-6 py-4 text-white"><?php echo $sale['percentage']; ?>%</td>
                                        <td class="px-6 py-4">
                                            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium <?php echo ($sale['status'] ?? 'active') === 'active' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'; ?>">
                                                <?php echo ucfirst($sale['status'] ?? 'active'); ?>
                                            </span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex gap-3">
                                                <button onclick='openModal("edit", <?php echo json_encode($sale); ?>)' class="text-blue-400 hover:text-blue-300 transition-colors" title="Edit">
                                                    <i class="bi bi-pencil-square text-lg"></i>
                                                </button>
                                                <form method="POST" onsubmit="return confirm('Are you sure you want to delete this sale?');" class="inline">
        ...
        <!-- Modal -->
        <div id="saleModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    ...
                    <form method="POST" id="saleForm">
                        ...
                        <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="flex justify-between items-center mb-5">
                                <h3 class="text-lg leading-6 font-medium text-white" id="modalTitle">Add New Sale</h3>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="name" id="saleName" required class="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors" placeholder="Summer Sale">
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium text-gray-400 mb-1">Percentage Off (%)</label>
                                    <input type="number" name="percentage" id="salePercentage" required min="0" max="100" class="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors">
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-gray-400 mb-1">Status</label>
                                    <select name="status" id="saleStatus" class="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors">
