...
try {
    ...
    foreach ($colCheck as $col) {
        if ($col['name'] === 'customer_phone')
            $hasPhoneCol = true;
        if ($col['name'] === 'customer_address')
            $hasAddressCol = true;
        if ($col['name'] === 'items')
            $hasItemsCol = true;
    }
...
foreach ($orders as $o) {
    ...
    elseif ($s === 'completed') {
        $completedOrders++;
        $revenue += floatval($o['total_amount']);
    } elseif ($s === 'cancelled')
        $cancelledOrders++;
}
...
<div class="flex flex-1 flex-col overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto overflow-x-hidden bg-[#1b1b1c] p-4 md:p-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <button onclick="openModal('add')"
                class="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 transition-colors font-medium text-sm shadow-lg shadow-purple-600/20">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-gray-800 rounded-xl border border-white/5 p-4">
                ...
                <p class="text-2xl font-bold text-white"><?php echo __CURRENCY_SIGN__ . number_format($revenue, 2); ?>
                </p>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
            ...
            <div class="flex flex-wrap gap-1 bg-gray-800 rounded-lg p-1 border border-white/5">
                <button onclick="filterOrders('all')" data-filter="all"
                    class="order-filter-tab active-tab px-3 py-1.5 rounded-md text-xs font-medium transition-colors">
                    All <span class="ml-1 opacity-70"><?php echo $totalOrders; ?></span>
                </button>
                <button onclick="filterOrders('pending')" data-filter="pending"
                    class="order-filter-tab px-3 py-1.5 rounded-md text-xs font-medium transition-colors text-gray-400 hover:text-white">
                    Pending <span class="ml-1 opacity-70"><?php echo $pendingOrders; ?></span>
                </button>
                <button onclick="filterOrders('processing')" data-filter="processing"
                    class="order-filter-tab px-3 py-1.5 rounded-md text-xs font-medium transition-colors text-gray-400 hover:text-white">
                    Processing <span class="ml-1 opacity-70"><?php echo $processingOrders; ?></span>
                </button>
                <button onclick="filterOrders('completed')" data-filter="completed"
                    class="order-filter-tab px-3 py-1.5 rounded-md text-xs font-medium transition-colors text-gray-400 hover:text-white">
                    Completed <span class="ml-1 opacity-70"><?php echo $completedOrders; ?></span>
                </button>
                <button onclick="filterOrders('cancelled')" data-filter="cancelled"
                    class="order-filter-tab px-3 py-1.5 rounded-md text-xs font-medium transition-colors text-gray-400 hover:text-white">
            ...
            <div class="relative w-full md:w-72">
                ...
                <input type="text" id="orderSearch" placeholder="Search by name or email..."
                            ...
                            <tr id="emptyRow">
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        <div
                                            class="w-16 h-16 rounded-full bg-gray-700/50 flex items-center justify-center mb-4">
                                            <i class="bi bi-bag-x text-3xl text-gray-500"></i>
                                        </div>
                                        <p class="text-gray-400 font-medium mb-1">No orders yet</p>
                                        <p class="text-gray-500 text-xs mb-4">Orders will appear here when customers make
                                            purchases</p>
                                        <button onclick="openModal('add')"
                                            class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-xs font-medium transition-colors inline-flex items-center gap-1.5">
                        ...
                        <?php else: ?>
                            <?php foreach ($orders as $order):
                                ...
                                <tr class="order-row hover:bg-white/[0.02] transition-colors cursor-pointer group"
                                    ...
                                    <td class="px-5 py-4">
                                        <span
                                            class="font-mono text-sm font-semibold text-white">#<?php echo str_pad($order['id'], 4, '0', STR_PAD_LEFT); ?></span>
                                        <?php if ($items): ?>
                                            <span class="ml-1.5 text-[10px] text-gray-500"><?php echo count($items); ?>
                                                item<?php echo count($items) !== 1 ? 's' : ''; ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-5 py-4">
                                    ...
                                    <td class="px-5 py-4">
                                        <span
                                            class="font-semibold text-white"><?php echo __CURRENCY_SIGN__ . number_format($order['total_amount'], 2); ?></span>
                                    </td>
                                    <td class="px-5 py-4" onclick="event.stopPropagation()">
                                        <div class="relative inline-block">
                                            <button onclick="toggleStatusDropdown(this, event)"
                                                class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium border <?php echo $sc; ?> hover:opacity-80 transition-opacity">
                                                <span class="w-1.5 h-1.5 rounded-full <?php echo $sd; ?>"></span>
                                                <?php echo ucfirst($status); ?>
                                                <i class="bi bi-chevron-down text-[10px] opacity-60"></i>
                                            </button>
                                            <div
                                                class="status-dropdown hidden absolute top-full left-0 mt-1 w-40 bg-gray-700 rounded-lg shadow-xl border border-white/10 z-30 py-1 overflow-hidden">
                                                <?php foreach (['pending', 'processing', 'completed', 'cancelled'] as $opt):
                                                    ...
                                                    <form method="POST" class="contents">
                                                        ...
                                                        <button type="submit"
                                                            class="w-full flex items-center gap-2 px-3 py-2 text-xs text-gray-200 hover:bg-white/10 transition-colors <?php echo $isActive; ?>">
                                    ...
                                    <td class="px-5 py-4 text-right" onclick="event.stopPropagation()">
                                        <div
                                            ...
                                            <button
                                                onclick='openModal("edit", <?php echo htmlspecialchars(json_encode($order)); ?>)'
                                                class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-blue-400 hover:bg-blue-400/10 transition-colors"
                                                title="Edit">
                                                <i class="bi bi-pencil text-sm"></i>
                                            </button>
                                            <button
                                                onclick="viewOrder(<?php echo htmlspecialchars(json_encode($order)); ?>, <?php echo $hasItemsCol ? 'true' : 'false'; ?>)"
                                                class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-purple-400 hover:bg-purple-400/10 transition-colors"
                                                title="View Details">
                                            ...
                                            <form method="POST" onsubmit="return confirm('Delete this order permanently?');"
                                                ...
                                                <input type="hidden" name="id" value="<?php echo $order['id']; ?>">
                                                <button type="submit"
                                                    class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-400 hover:bg-red-400/10 transition-colors"
                                                    title="Delete">
...
<div id="orderModal" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
        <div class="fixed inset-0 bg-[#252526] /60 backdrop-blur-sm transition-opacity" onclick="closeModal()"></div>
        <div
            class="relative w-full max-w-lg bg-gray-800 rounded-xl shadow-2xl border border-white/10 transform transition-all">
            <form method="POST" id="orderForm">
                ...
                <div class="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-400 mb-1.5">Total Amount
                                (<?php echo __CURRENCY_SIGN__; ?>) *</label>
                ...
                <div class="flex justify-end gap-3 px-6 py-4 border-t border-white/5 bg-gray-800/50">
                    <button type="button" onclick="closeModal()"
                        class="px-4 py-2.5 rounded-lg border border-gray-600 text-sm font-medium text-gray-300 hover:text-white hover:bg-gray-700 transition-colors">
                        Cancel
                    </button>
                    <button type="submit"
                        class="px-5 py-2.5 rounded-lg bg-purple-600 text-sm font-medium text-white hover:bg-purple-700 transition-colors shadow-lg shadow-purple-600/20">
                        Save Order
                    </button>
                </div>
            </form>
...
<div id="detailModal" class="fixed inset-0 z-50 hidden" role="dialog" aria-modal="true">
    <div class="flex items-center justify-center min-h-screen px-4 py-6">
        <div class="fixed inset-0 bg-[#252526] /60 backdrop-blur-sm transition-opacity" onclick="closeDetailModal()">
        </div>
        <div
            class="relative w-full max-w-xl bg-gray-800 rounded-xl shadow-2xl border border-white/10 transform transition-all">
            ...
            <div class="px-6 py-5 max-h-[65vh] overflow-y-auto space-y-5">
                ...
                <div class="bg-gray-700/30 rounded-lg p-4 border border-white/5">
                    <h4 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Customer Information
                    </h4>
                ...
                <div id="detailItemsSection" class="hidden">
                    <div class="bg-gray-700/30 rounded-lg border border-white/5 overflow-hidden">
                        <h4 class="text-xs font-semibold text-gray-400 uppercase tracking-wider px-4 pt-4 pb-2">Order
                            Items</h4>
            ...
            <div class="flex justify-end gap-3 px-6 py-4 border-t border-white/5 bg-gray-800/50">
                ...
                <button id="detailEditBtn"
                    class="px-5 py-2.5 rounded-lg bg-purple-600 text-sm font-medium text-white hover:bg-purple-700 transition-colors">
