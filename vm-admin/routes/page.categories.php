    ...
    <main class="flex-1 overflow-y-auto overflow-x-hidden bg-[#1b1b1c] p-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <button onclick="openModal('add')"
                class="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-lg font-medium text-sm transition-colors shadow-lg shadow-purple-600/20">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-gray-800 rounded-xl border border-white/5 p-5">
                <div class="flex items-center gap-3">
                    <div
                        class="flex items-center justify-center h-10 w-10 rounded-lg bg-emerald-600/20 text-emerald-400">
        ...
        <div class="mb-6">
            <div class="relative max-w-md">
                ...
                <input type="text" id="categorySearch" placeholder="Search categories..." oninput="filterCategories()"
                    class="w-full bg-gray-800 border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors">
            </div>
        </div>
        ...
        <?php if (empty($categories)): ?>
            <div class="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p class="text-sm text-gray-500 mb-6 max-w-sm">Categories help your customers find products faster. Create
                    your first category to get started.</p>
                <button onclick="openModal('add')"
                    class="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-lg font-medium text-sm transition-colors">
        ...
        <?php else: ?>
            <div id="categoriesGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                <?php foreach ($categories as $category): ?>
                    <div class="category-card group relative bg-gray-800 rounded-xl border border-white/5 overflow-hidden hover:border-purple-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/5"
                        data-name="<?php echo htmlspecialchars(strtolower($category['name'])); ?>"
                        data-desc="<?php echo htmlspecialchars(strtolower($category['description'] ?? '')); ?>">
                        <!-- Card Image -->
                        <div class="relative aspect-[16/9] bg-[#1b1b1c] overflow-hidden">
                            ...
                            <!-- Product count badge -->
                            <div
                                class="absolute top-3 right-3 bg-[#1b1b1c]/80 backdrop-blur-sm text-white text-xs font-bold px-2.5 py-1 rounded-full border border-white/10">
                                <?php echo (int) $category['product_count']; ?>
                                product<?php echo (int) $category['product_count'] !== 1 ? 's' : ''; ?>
                            </div>

                            <!-- Hover overlay with actions -->
                            <div
                                class="absolute inset-0 bg-[#1b1b1c]/70 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3">
                                <button onclick='openModal("edit", <?php echo json_encode($category); ?>)'
                                ...
                                <form method="POST"
                                    onsubmit="return confirm('Delete this category? Products inside will become uncategorized.');"
                                    class="inline">
                                    <input type="hidden" name="action" value="delete_category">
                                    <input type="hidden" name="id" value="<?php echo $category['id']; ?>">
                                    <button type="submit"
                                        class="flex items-center gap-2 bg-red-600/80 hover:bg-red-600 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
...
<div id="categoryModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4 pointer-events-none">
        <div id="modalPanel"
            class="pointer-events-auto w-full max-w-lg bg-gray-800 rounded-xl border border-white/10 shadow-2xl transform transition-all duration-300 scale-95 opacity-0">
            <form method="POST" id="categoryForm">
                ...
                <div class="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="categoryName" required
                            ...
                            class="w-full bg-gray-700 border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="categoryDescription" rows="3"
                            ...
                            class="w-full bg-gray-700 border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors resize-none"></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <div id="imagePreviewContainer"
                            class="hidden mb-3 relative w-full aspect-[16/9] rounded-xl overflow-hidden bg-[#1b1b1c] border border-white/10 group">
                            <img id="imagePreview" src="" class="h-full w-full object-cover">
                            <button type="button" onclick="removeImage()"
                                class="absolute top-2 right-2 bg-red-500/90 hover:bg-red-500 text-white rounded-lg p-1.5 opacity-0 group-hover:opacity-100 transition-all">
                        ...
                        <label class="relative block cursor-pointer">
                            <div
                                class="flex flex-col items-center justify-center gap-2 bg-gray-700/50 border-2 border-dashed border-white/10 rounded-xl px-4 py-8 text-gray-400 hover:border-purple-500/50 hover:text-gray-300 transition-all">
                        ...
                        <div class="flex items-center gap-3 my-3">
                            ...
                            <span class="text-[10px] text-gray-600 font-bold uppercase tracking-widest">or paste
                                URL</span>
                            <div class="h-px flex-1 bg-white/5"></div>
                        </div>

                        <!-- URL Input -->
                        <input type="text" name="image" id="categoryImage" placeholder="https://example.com/image.jpg"
                ...
                <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/5 bg-gray-800/50">
                    <button type="button" onclick="closeModal()"
                        class="px-4 py-2 rounded-lg border border-white/10 text-sm font-medium text-gray-300 hover:text-white hover:bg-white/5 transition-colors">
                        Cancel
                    </button>
                    <button type="submit"
                        class="px-5 py-2 rounded-lg bg-purple-600 hover:bg-purple-700 text-sm font-medium text-white transition-colors shadow-lg shadow-purple-600/20">
                        Save Category
                    </button>
                </div>
            </form>
