...
<div class="bg-[#252526] border border-zinc-800 rounded-xl overflow-hidden">
    <div class="px-5 py-4 border-b border-zinc-800">
        ...
        <p class="text-zinc-400 text-sm mt-1">Connect your desktop or mobile application to manage this store remotely
        </p>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="bg-zinc-800 rounded-xl p-4">
                <div class="flex items-center gap-3 mb-3">
                    <span
                        class="w-7 h-7 rounded-full bg-violet-600 text-white flex items-center justify-center text-xs font-bold">1</span>
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="#"
                        class="flex items-center gap-2 bg-[#252526] hover:bg-zinc-700 px-3 py-1.5 rounded-lg transition-colors text-xs text-white">
            ...
            <div class="bg-zinc-800 rounded-xl p-4">
                <div class="flex items-center gap-3 mb-3">
                    <span
                        class="w-7 h-7 rounded-full bg-violet-600 text-white flex items-center justify-center text-xs font-bold">2</span>
                ...
                <div class="space-y-2">
                    <div class="bg-[#252526] rounded-lg px-3 py-2">
                        <span class="text-zinc-500 text-[10px] uppercase tracking-wider">Store URL</span>
                        <div class="flex items-center justify-between mt-0.5">
                            <code
                                class="text-white font-mono text-xs break-all"><?= htmlspecialchars($console_store_url) ?></code>
                            <button
                                onclick="navigator.clipboard.writeText('<?= htmlspecialchars($console_store_url) ?>')"
                                class="text-zinc-500 hover:text-white text-xs ml-2"><i class="bi bi-copy"></i></button>
                        </div>
                    </div>
                    <div class="bg-[#252526] rounded-lg px-3 py-2">
                        ...
                        <div class="flex items-center justify-between mt-0.5">
                            ...
                            <button
                                onclick="navigator.clipboard.writeText('<?= htmlspecialchars($console_store_id) ?>')"
                                class="text-zinc-500 hover:text-white text-xs ml-2"><i class="bi bi-copy"></i></button>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-zinc-500 text-xs mt-1 mb-3">This key authenticates your app with this store. Keep it
                        secure.</p>
                    <div class="bg-[#252526] rounded-lg px-3 py-2">
                        <div class="flex items-center justify-between flex-wrap gap-2">
                            <code class="text-white font-mono text-sm break-all"
                                id="connection_secret"><?= htmlspecialchars($console_secret) ?></code>
                            <div class="flex items-center gap-2">
                                ...
                                <form method="POST" style="display:inline"
                                    onsubmit="return confirm('Regenerating will disconnect all connected apps. Continue?')">
                                    <input type="hidden" name="action" value="save_console">
                                    <input type="hidden" name="console[regenerate_secret]" value="1">
                                    <button type="submit"
                                        class="bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 px-3 py-1 rounded-lg text-xs transition-colors">
