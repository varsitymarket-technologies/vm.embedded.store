    ...
    <div class="bg-[#252526] border border-zinc-800 rounded-xl overflow-hidden">
        ...
        <div class="p-5 space-y-6">
            ...
            <div class="bg-zinc-800 rounded-xl overflow-hidden border border-zinc-700">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="flex items-center gap-2">
                                    ...
                                    <?php else: ?>
                                        <span
                                            class="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-zinc-700 text-zinc-400">Inactive</span>
                                    <?php endif; ?>
                                </div>
                                <p class="text-zinc-500 text-xs mt-0.5">Real-time store notifications to your Discord
                                    server</p>
                            </div>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="hidden" name="discord[enabled]" value="0">
                            <input type="checkbox" name="discord[enabled]" value="1" class="sr-only peer"
                                id="discord_toggle" <?= $discord_enabled === '1' ? 'checked' : '' ?>>
                            <div
                ...
                <div id="discord_config" class="p-4 space-y-5"
                    style="opacity:<?= $discord_enabled === '1' ? '1' : '0.5' ?>;pointer-events:<?= $discord_enabled === '1' ? 'auto' : 'none' ?>">
                    ...
                    <div>
                        ...
                        <input type="text" name="discord[webhook_url]" value="<?= htmlspecialchars($discord_webhook) ?>"
                            ...
                            class="w-full bg-[#252526] border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors font-mono">
                        <p class="text-zinc-500 text-xs mt-1"><i class="bi bi-info-circle"></i> Server Settings &rarr;
                            Integrations &rarr; Webhooks &rarr; New Webhook</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            <?php foreach ($discord_event_list as $evt_key => $evt): ?>
                                <label
                                    class="flex items-center gap-3 bg-[#252526] rounded-lg px-3 py-2.5 cursor-pointer hover:bg-[#252526]/80 transition-colors">
                                    <input type="checkbox" name="discord_events[]" value="<?= $evt_key ?>"
                                        class="w-4 h-4 accent-indigo-500 rounded" <?= in_array($evt_key, $discord_events) ? 'checked' : '' ?>>
                                    <div>
                                        <span
                                            class="text-white text-xs font-medium"><?= htmlspecialchars($evt['label']) ?></span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="discord[message_style]"
                                class="w-full bg-[#252526] border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors">
                                <option value="embed" <?= $discord_style === 'embed' ? 'selected' : '' ?>>Rich Embed
                                </option>
                                <option value="simple" <?= $discord_style === 'simple' ? 'selected' : '' ?>>Simple Text
                                </option>
                                <option value="compact" <?= $discord_style === 'compact' ? 'selected' : '' ?>>Compact
                                </option>
                            </select>
                        </div>
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                <input type="color" name="discord[embed_color]"
                                    value="<?= htmlspecialchars($discord_color) ?>"
                                    class="w-10 h-8 rounded cursor-pointer bg-[#252526] border border-zinc-700">
                                <span
                                    class="text-white text-xs font-mono"><?= htmlspecialchars($discord_color) ?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="bg-[#252526] rounded-lg p-3 space-y-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="discord_mentions[]" value="everyone"
                                    class="w-4 h-4 accent-indigo-500 rounded" <?= in_array('everyone', $discord_mentions) ? 'checked' : '' ?>>
                                <span class="text-white text-xs">@everyone for important orders</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="discord_mentions[]" value="here"
                                    class="w-4 h-4 accent-indigo-500 rounded" <?= in_array('here', $discord_mentions) ? 'checked' : '' ?>>
                                <span class="text-white text-xs">@here for low stock alerts</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="discord_mentions[]" value="role"
                                    class="w-4 h-4 accent-indigo-500 rounded" <?= in_array('role', $discord_mentions) ? 'checked' : '' ?>>
                    ...
                    <details>
                        <summary
                            class="cursor-pointer text-zinc-400 hover:text-white text-xs font-medium transition-colors">
                            <i class="bi bi-gear"></i> Advanced Settings
                        </summary>
                        <div class="mt-3 space-y-3 pl-4 border-l-2 border-zinc-700">
                            <div>
                                <label class="block text-zinc-400 text-xs font-medium mb-1.5">Custom Webhook
                                    Name</label>
                                <input type="text" name="discord[bot_name]"
                                    value="<?= htmlspecialchars($discord_bot_name) ?>" placeholder="Store Notifications"
                                    class="w-full bg-[#252526] border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors">
                            </div>
                            <div>
                                <label class="block text-zinc-400 text-xs font-medium mb-1.5">Custom Avatar URL</label>
                                <input type="url" name="discord[avatar_url]"
                                    value="<?= htmlspecialchars($discord_avatar) ?>"
            ...
            <!-- More Plugins -->
            <div
                class="flex flex-col items-center justify-center py-10 bg-zinc-800 rounded-xl border border-dashed border-zinc-700">
