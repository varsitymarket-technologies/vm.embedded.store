    ...
    <div class="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 mb-4">
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
                ...
                <p class="text-zinc-400 text-xs mt-1 mb-2">Copy this key now. You won't be able to see the full key again.
                </p>
                <div class="flex items-center gap-2 bg-[#252526] rounded-lg px-3 py-2">
                    <code id="newKeyValue"
                        class="text-sm font-mono text-white break-all flex-1"><?php echo htmlspecialchars($new_key_display, ENT_QUOTES, 'UTF-8'); ?></code>
                    <button
                        onclick="navigator.clipboard.writeText(document.getElementById('newKeyValue').textContent); this.innerHTML='<i class=\'bi bi-check-lg\'></i> Copied'"
...
<div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div class="bg-violet-500/5 border border-violet-500/20 rounded-lg px-4 py-3">
                    ...
                    <div class="flex items-center justify-between mt-1">
                        <code
                            class="text-white font-mono text-sm"><?php echo htmlspecialchars($store_id, ENT_QUOTES, 'UTF-8'); ?></code>
                        <button
                            onclick="navigator.clipboard.writeText('<?php echo htmlspecialchars($store_id, ENT_QUOTES, 'UTF-8'); ?>'); this.innerHTML='<i class=\'bi bi-check-lg\'></i>'; setTimeout(() => this.innerHTML='<i class=\'bi bi-copy\'></i>', 2000)"
                            class="text-zinc-500 hover:text-white text-xs transition-colors"><i
                ...
                <div class="bg-sky-500/5 border border-sky-500/20 rounded-lg px-4 py-3">
                    ...
                    <div class="flex items-center justify-between mt-1">
                        <code id="apiEndpoint"
                            class="text-white font-mono text-sm break-all"><?php echo htmlspecialchars($api_base_url, ENT_QUOTES, 'UTF-8'); ?></code>
                        <button
                            onclick="navigator.clipboard.writeText(document.getElementById('apiEndpoint').textContent); this.innerHTML='<i class=\'bi bi-check-lg\'></i>'; setTimeout(() => this.innerHTML='<i class=\'bi bi-copy\'></i>', 2000)"
                            class="text-zinc-500 hover:text-white text-xs transition-colors ml-2"><i
                                class="bi bi-copy"></i></button>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-zinc-800 rounded-lg p-4 hidden">
                ...
                <div class="space-y-1.5 text-xs">
                    ...
                    <div class="flex items-center gap-2"><span
                            class="bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-mono text-[10px]">GET</span><code
                            class="text-zinc-400 font-mono">?state=products</code><span class="text-zinc-600">— List
                            products</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-mono text-[10px]">GET</span><code
                            class="text-zinc-400 font-mono">?state=product&id={id}</code><span class="text-zinc-600">—
                            Single product</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-mono text-[10px]">GET</span><code
                            class="text-zinc-400 font-mono">?state=categories</code><span class="text-zinc-600">— List
                            categories</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-mono text-[10px]">GET</span><code
                            class="text-zinc-400 font-mono">?state=search&q={query}</code><span class="text-zinc-600">—
                            Search products</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-mono text-[10px]">GET</span><code
                            class="text-zinc-400 font-mono">?state=discounts</code><span class="text-zinc-600">— Active
                            discounts</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-mono text-[10px]">GET</span><code
                            class="text-zinc-400 font-mono">?state=site</code><span class="text-zinc-600">— Store
                            info</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-mono text-[10px]">GET</span><code
                            class="text-zinc-400 font-mono">?state=orders&email={email}</code><span
                            class="text-zinc-600">— Order history</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-emerald-500/10 text-emerald-400 px-1.5 py-0.5 rounded font-mono text-[10px]">GET</span><code
                            class="text-zinc-400 font-mono">?state=cart&cart_id={id}</code><span class="text-zinc-600">—
                            Cart contents</span></div>

                    <p class="text-zinc-500 text-[10px] uppercase tracking-wider font-medium mt-3 mb-1">Cart & Checkout
                    </p>
                    <div class="flex items-center gap-2"><span
                            class="bg-sky-500/10 text-sky-400 px-1.5 py-0.5 rounded font-mono text-[10px]">POST</span><code
                            class="text-zinc-400 font-mono">?state=cart_create</code><span class="text-zinc-600">—
                            Create cart</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-sky-500/10 text-sky-400 px-1.5 py-0.5 rounded font-mono text-[10px]">POST</span><code
                            class="text-zinc-400 font-mono">?state=cart_add</code><span class="text-zinc-600">— Add to
                            cart</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-sky-500/10 text-sky-400 px-1.5 py-0.5 rounded font-mono text-[10px]">POST</span><code
                            class="text-zinc-400 font-mono">?state=cart_update</code><span class="text-zinc-600">—
                            Update quantity</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-sky-500/10 text-sky-400 px-1.5 py-0.5 rounded font-mono text-[10px]">POST</span><code
                            class="text-zinc-400 font-mono">?state=cart_remove</code><span class="text-zinc-600">—
                            Remove item</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-sky-500/10 text-sky-400 px-1.5 py-0.5 rounded font-mono text-[10px]">POST</span><code
                            class="text-zinc-400 font-mono">?state=checkout_create</code><span class="text-zinc-600">—
                            Create checkout</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-sky-500/10 text-sky-400 px-1.5 py-0.5 rounded font-mono text-[10px]">POST</span><code
                            class="text-zinc-400 font-mono">?state=checkout_complete</code><span class="text-zinc-600">—
                            Complete checkout</span></div>
                    <div class="flex items-center gap-2"><span
                            class="bg-sky-500/10 text-sky-400 px-1.5 py-0.5 rounded font-mono text-[10px]">POST</span><code
                            class="text-zinc-400 font-mono">?state=order</code><span class="text-zinc-600">— Place
                            order</span></div>
                </div>
                <p class="text-zinc-600 text-xs mt-3">Auth via <code class="text-zinc-500">X-API-Key</code> header,
                    <code class="text-zinc-500">Authorization: Bearer {key}</code>, or <code
                        class="text-zinc-500">?api_key={key}</code></p>
            </div>
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-zinc-800/50">
                        <?php foreach ($api_keys as $key): ?>
                            <tr
                                class="hover:bg-zinc-800/30 transition-colors group <?php echo $key['active'] ? '' : 'opacity-50'; ?>">
                                <td class="px-5 py-3 text-white text-sm">
                                    <?php echo htmlspecialchars($key['key_name'], ENT_QUOTES, 'UTF-8'); ?></td>
                                <td class="px-5 py-3">
                                    <code
                                        class="bg-zinc-800 px-2 py-0.5 rounded text-xs font-mono text-zinc-300"><?php echo htmlspecialchars(substr($key['api_key'], 0, 12), ENT_QUOTES, 'UTF-8'); ?>••••</code>
                                </td>
                                <td class="px-5 py-3">
                                    <?php if ($key['active']): ?>
                                        <span
                                            class="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400">
                                            <span class="w-1 h-1 rounded-full bg-emerald-400"></span> Active
                                        </span>
                                    <?php else: ?>
                                        <span
                                            class="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-red-500/10 text-red-400">
                                            <span class="w-1 h-1 rounded-full bg-red-400"></span> Revoked
                                        </span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-5 py-3 text-zinc-500 text-xs">
                                    <?php echo htmlspecialchars($key['created_at'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
                                <td class="px-5 py-3 text-zinc-500 text-xs">
                                    <?php echo $key['last_used'] ? htmlspecialchars($key['last_used'], ENT_QUOTES, 'UTF-8') : 'Never'; ?>
                                </td>
                                    ...
                                    <div class="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <?php if ($key['active']): ?>
                                            <form method="POST" class="inline" onsubmit="return confirm('Revoke this API key?')">
                                                ...
                                                <button type="submit"
                                                    class="p-1.5 rounded-md hover:bg-zinc-700 text-amber-400 transition-colors"
                                                    title="Revoke"><i class="bi bi-slash-circle"></i></button>
                                            </form>
                                        <?php endif; ?>
    ...
    <div class="bg-[#252526] border border-zinc-800 rounded-xl overflow-hidden">
        <div class="px-5 py-4 border-b border-zinc-800">
            ...
            <p class="text-zinc-500 text-xs mt-0.5">Only these domains can make browser API requests. Empty = allow all.
            </p>
        </div>
        <div class="p-5">
            <form method="POST" class="flex items-center gap-2 mb-4">
                ...
                <button type="submit"
                    class="bg-violet-600 hover:bg-violet-500 text-white px-4 py-2 rounded-lg text-xs font-medium transition-colors whitespace-nowrap">
                    ...
                    <?php foreach ($cors_domains as $cd): ?>
                        <div class="flex items-center justify-between bg-zinc-800 rounded-lg px-4 py-2.5">
                            <div class="flex items-center gap-2">
                                ...
                                <code
                                    class="text-sm font-mono text-white"><?php echo htmlspecialchars($cd['domain'], ENT_QUOTES, 'UTF-8'); ?></code>
                            </div>
                            <form method="POST" class="inline" onsubmit="return confirm('Remove this domain?')">
                                ...
                                <button type="submit" class="text-red-400 hover:text-red-300 text-xs transition-colors"><i
            ...
            <?php else: ?>
                <div class="bg-amber-500/5 border border-amber-500/15 rounded-lg p-3 flex items-start gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-zinc-400 text-[10px] mt-0.5">All origins (<code class="text-zinc-500">*</code>)
                            currently allowed. Add domains to restrict.</p>
                    </div>
                </div>
            <?php endif; ?>
        </div>
    ...
    <!-- JavaScript SDK -->
    <div class="bg-[#252526] border border-zinc-800 rounded-xl overflow-hidden hidden">
        <div class="px-5 py-4 border-b border-zinc-800">
        ...
        <div class="p-5 space-y-4">
            ...
            <div class="bg-violet-500/5 border border-violet-500/20 rounded-lg px-4 py-3">
                ...
                <div class="flex items-center gap-2 mt-1">
                    <code id="sdkScriptTag"
                        class="text-sm font-mono text-violet-300 break-all flex-1">&lt;script src="<?php echo htmlspecialchars($sdk_url, ENT_QUOTES, 'UTF-8'); ?>"&gt;&lt;/script&gt;</code>
                    <button
                        onclick="navigator.clipboard.writeText('<script src=\'<?php echo htmlspecialchars($sdk_url, ENT_QUOTES, 'UTF-8'); ?>\'><\/script>'); this.innerHTML='<i class=\'bi bi-check-lg\'></i>'; setTimeout(() => this.innerHTML='<i class=\'bi bi-copy\'></i>', 2000)"
                        class="text-zinc-500 hover:text-white text-xs transition-colors"><i
                            class="bi bi-copy"></i></button>
                </div>
            </div>
            ...
            <div class="bg-zinc-800 rounded-lg p-4">
                ...
                <div class="space-y-1.5 text-[11px] font-mono">
                    <div class="flex items-start gap-2"><span
                            class="bg-violet-500/10 text-violet-400 px-1.5 py-0.5 rounded shrink-0">Products</span><code
                            class="text-zinc-400">.products.list() / .get(id) / .search(q) / .byCategory(id)</code>
                    </div>
                    <div class="flex items-start gap-2"><span
                            class="bg-violet-500/10 text-violet-400 px-1.5 py-0.5 rounded shrink-0">Cart</span><code
                            class="text-zinc-400">.cart.add(id, qty) / .update(id, qty) / .remove(id) / .get() / .clear()</code>
                    </div>
                    <div class="flex items-start gap-2"><span
                            class="bg-violet-500/10 text-violet-400 px-1.5 py-0.5 rounded shrink-0">Checkout</span><code
                            class="text-zinc-400">.checkout.redirect({returnUrl}) / .create() / .complete()</code></div>
                    <div class="flex items-start gap-2"><span
                            class="bg-violet-500/10 text-violet-400 px-1.5 py-0.5 rounded shrink-0">UI</span><code
                            class="text-zinc-400">.ui.productGrid(el) / .cartBadge(el) / .cartDrawer(el) / .injectStyles()</code>
                    </div>
                    <div class="flex items-start gap-2"><span
                            class="bg-violet-500/10 text-violet-400 px-1.5 py-0.5 rounded shrink-0">Events</span><code
                            class="text-zinc-400">.on('cart:updated' | 'cart:item-added' | 'checkout:completed', cb)</code>
                    </div>
                </div>
            </div>
...
<div id="generateKeyModal" class="hidden fixed inset-0 z-50">
    <div class="flex items-center justify-center min-h-screen px-4">
        <div class="fixed inset-0 bg-[#252526] /60 backdrop-blur-sm"
