...
<div class="min-h-screen bg-[#252526] text-zinc-300 p-6 font-sans selection:bg-violet-500/30">
    <a href="?tab=general"
        class="inline-flex items-center gap-2 text-zinc-500 hover:text-violet-400 text-sm font-medium transition-colors mb-8">
    ...
    <form method="POST" id="domain-setup-form" class="max-w-6xl mx-auto flex flex-col lg:flex-row gap-8">
        ...
        <div class="flex-1">
            ...
            <div class="mb-8">
                ...
                <div class="flex items-center justify-between relative">
                    ...
                    <button type="button" class="step-indicator flex flex-col items-center gap-2" data-target="1">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center border-2 bg-[#252526]  border-violet-500 text-violet-400 font-bold transition-all"
                            id="ind-1">1</div>
                    ...
                    <button type="button" class="step-indicator flex flex-col items-center gap-2" data-target="2">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center border-2 bg-[#252526]  border-zinc-800 text-zinc-600 font-bold transition-all"
                            id="ind-2">2</div>
            ...
            <div id="step-1" class="wizard-step bg-[#252526] border border-zinc-800 rounded-xl p-6 shadow-lg">
                <h3 class="text-lg font-bold text-white mb-1">Custom Domain</h3>
                <p class="text-zinc-500 text-sm mb-6">Link your own branding by routing your registered domain name to
                    our engine cluster.</p>

                <div class="space-y-6">
                    <div>
                        <label class="block text-zinc-400 text-xs font-bold uppercase tracking-wider mb-2">Domain
                            Name</label>
                        <div class="relative">
                            <span
                                class="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-600 text-sm font-mono">https://</span>
                            <input type="text" name="domain[name]" id="input-domain-name"
                                value="<?= htmlspecialchars($domain_input) ?>" placeholder="yourstore.com"
                                class="w-full bg-[#252526]  border border-zinc-700 rounded-lg pl-20 pr-4 py-3 text-white text-sm font-mono focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-colors">
                        </div>
                        <p class="text-zinc-500 text-xs mt-2">Enter your root domain or standard subdomain (do not
                            include paths or trailing slashes).</p>
                    </div>

                    <div class="bg-[#252526]  border border-zinc-800/50 rounded-lg p-4">
                        <label class="flex items-start gap-3 cursor-pointer group">
                            <input type="checkbox" name="domain[www_redirect]" value="1" id="input-www-redirect"
                                class="w-4 h-4 mt-0.5 bg-[#252526]  border-zinc-700 accent-violet-600 rounded"
                                <?= $is_www_redirect === '1' ? 'checked' : '' ?>>
                            <span class="text-sm select-none">
                                <span
                                    class="block text-zinc-300 font-medium group-hover:text-white transition-colors">Auto-forward
                                    WWW variant</span>
                                <span class="block text-zinc-500 text-xs mt-0.5">Enabling this seamlessly captures
                                    traffic sent to both <span class="font-mono">www.yourstore.com</span> and your root
                                    domain.</span>
                            </span>
                        </label>
                    </div>
                ...
                <div class="mt-8 flex justify-end">
                    <button type="button"
                        class="btn-next bg-violet-600 hover:bg-violet-500 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition-colors"
                        data-next="2">
            ...
            <div id="step-2" class="wizard-step hidden bg-[#252526] border border-zinc-800 rounded-xl p-6 shadow-lg">
                <h3 class="text-lg font-bold text-white mb-1">DNS Zone Alignment</h3>
                <p class="text-zinc-500 text-sm mb-6">Log in to your DNS registrar account (e.g., GoDaddy, Namecheap,
                    Cloudflare) and append the following parameters to your zone file.</p>

                <div class="space-y-4">
                    <div class="bg-[#252526]  border border-zinc-800 rounded-lg p-4 relative overflow-hidden">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div class="grid grid-cols-3 gap-4 flex-1">
                                <div>
                                    <span
                                        class="block text-zinc-500 text-[10px] font-bold uppercase tracking-wider">Type</span>
                                    <span class="text-violet-400 font-mono text-sm font-bold">A</span>
                                </div>
                                <div>
                                    <span
                                        class="block text-zinc-500 text-[10px] font-bold uppercase tracking-wider">Host</span>
                                    <span class="text-white font-mono text-sm">@</span>
                                </div>
                                <div>
                                    <span
                                        class="block text-zinc-500 text-[10px] font-bold uppercase tracking-wider">Value
                                        (IPv4 Target)</span>
                                    <span class="text-white font-mono text-sm"
                                        id="dns-target-ip"><?= $engine_ipv4 ?></span>
                                </div>
                            </div>
                            <button type="button" onclick="copyText('dns-target-ip', this)"
                                class="inline-flex items-center justify-center gap-1.5 bg-[#252526] hover:bg-zinc-800 border border-zinc-700 text-xs text-zinc-400 hover:text-white px-3 py-1.5 rounded transition-all">
                                <i class="bi bi-clipboard"></i> <span>Copy IP</span>
                            </button>
                        </div>
                    </div>

                    <div id="dns-cname-panel"
                        class="bg-[#252526]  border border-zinc-800 rounded-lg p-4 relative overflow-hidden transition-all">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div class="grid grid-cols-3 gap-4 flex-1">
                                <div>
                                    <span
                                ...
                                <div>
                                    <span
                                        class="block text-zinc-500 text-[10px] font-bold uppercase tracking-wider">Value</span>
                                    <span class="text-white font-mono text-sm" id="dns-target-cname">@</span>
                                </div>
                            </div>
                            <button type="button" onclick="copyText('dns-target-cname', this)"
                                class="inline-flex items-center justify-center gap-1.5 bg-[#252526] hover:bg-zinc-800 border border-zinc-700 text-xs text-zinc-400 hover:text-white px-3 py-1.5 rounded transition-all">
                ...
                <div class="mt-6 border-t border-zinc-800 pt-4 flex items-start gap-2 text-zinc-500 text-xs">
                    ...
                    <span>DNS records can take anywhere from a few minutes to 24 hours to propagate internationally
                        across authoritative resolvers.</span>
                </div>

                <div class="mt-8 flex justify-between">
                    <button type="button"
                        class="btn-prev text-zinc-400 hover:text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-colors"
                        data-prev="1">
            ...
            <div id="step-3" class="wizard-step hidden bg-[#252526] border border-zinc-800 rounded-xl p-6 shadow-lg">
                <h3 class="text-lg font-bold text-white mb-1">Encryption Layer & Handshake</h3>
                <p class="text-zinc-500 text-sm mb-6">Select automated enforcement mechanisms for Edge SSL Certificates.
                </p>

                <div class="space-y-4 mb-8">
                    <div>
                        <label class="block text-zinc-400 text-xs font-bold uppercase tracking-wider mb-2">SSL
                            Deployment Strategy</label>
                        <select name="domain[ssl_mode]" id="input-ssl-mode"
                            class="w-full bg-[#252526]  border border-zinc-700 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-violet-500 transition-colors">
                            <option value="strict" <?= $ssl_mode === 'strict' ? 'selected' : '' ?>>Strict HTTPS Enforcement
                                (Recommended)</option>
                            <option value="flexible" <?= $ssl_mode === 'flexible' ? 'selected' : '' ?>>Flexible Routing
                                (HTTP & HTTPS proxy mapping)</option>
                        </select>
                    </div>
                    ...
                    <div class="p-4 bg-zinc-950 border border-zinc-800 rounded-lg">
                        <div class="flex items-center gap-3 text-xs text-zinc-400">
                            ...
                            <span>Engine reverse proxy validation pipeline is listening for downstream
                                connections.</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 flex justify-between">
                    ...
                    <button type="submit"
                        class="bg-violet-600 hover:bg-violet-500 text-white px-8 py-2.5 rounded-lg text-sm font-bold shadow-[0_0_15px_rgba(139,92,246,0.4)] transition-all">
        ...
        <div class="w-full lg:w-1/3">
            <div
                class="sticky top-6 bg-[#252526]  border border-zinc-800 rounded-xl overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                <div class="bg-[#252526] border-b border-zinc-800 px-4 py-3 flex items-center justify-between">
                    ...
                    <div
                        class="bg-[#252526] /40 px-3 py-1 rounded text-[11px] font-mono text-zinc-400 flex items-center gap-1.5 w-48 overflow-hidden whitespace-nowrap text-ellipsis border border-zinc-800/30">
                ...
                <div class="p-6 space-y-6">
                    <div class="bg-[#252526] rounded-lg p-5 border border-zinc-800/50">
                        <h4 class="text-xs font-bold uppercase tracking-wider text-zinc-500 mb-4">Topology Simulation
                        </h4>

                        <div class="space-y-4 font-mono text-xs relative">
                            <div class="absolute left-3 top-3 bottom-3 w-0.5 bg-dashed border-l border-zinc-800 -z-0">
                            </div>

                            <div class="flex items-center gap-3 relative z-10">
                                <div
                                    class="w-6 h-6 rounded-full bg-zinc-800 border border-zinc-700 flex items-center justify-center text-zinc-400">
                                    <i class="bi bi-globe text-[11px]"></i>
                                </div>
                                <div>
                                    <span class="block text-zinc-400 text-[11px]">External Alias</span>
                                    <span class="text-white font-bold text-xs"
                                        id="node-custom-domain">yourstore.com</span>
                                </div>
                            </div>

                            <div class="flex items-center gap-3 relative z-10">
                                <div
                                    class="w-6 h-6 rounded-full bg-violet-950 border border-violet-800 flex items-center justify-center text-violet-400 shadow-[0_0_8px_rgba(139,92,246,0.2)]">
                            ...
                            <div class="flex items-center gap-3 relative z-10">
                                <div
                                    class="w-6 h-6 rounded-full bg-[#252526] border border-zinc-800 flex items-center justify-center text-zinc-500">
    ...
    function goToStep(stepNumber) {
        ...
        document.querySelectorAll('.step-indicator').forEach((ind, index) => {
            ...
            if (index + 1 === stepNumber) {
                circle.className = "w-8 h-8 rounded-full flex items-center justify-center border-2 bg-[#252526]  border-violet-500 text-violet-400 font-bold transition-all shadow-[0_0_10px_rgba(139,92,246,0.3)]";
                ...
                circle.className = "w-8 h-8 rounded-full flex items-center justify-center border-2 bg-[#252526]  border-zinc-800 text-zinc-600 font-bold transition-all";
