...
<div class="min-h-screen bg-[#252526] text-zinc-300 p-6 font-sans selection:bg-violet-500/30">
    ...
    <form method="POST" id="currency-setup-form" class="max-w-6xl mx-auto flex flex-col lg:flex-row gap-8">
        ...
        <div class="w-full lg:w-1/3">
            <div
                class="sticky top-6 border border-zinc-800 rounded-xl overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                ...
                <div class="p-6">
                    <div class="bg-[#252526] rounded-lg p-1 border border-zinc-800/50 shadow-inner">
                        <div class="bg-[#252526]  rounded-md p-5 border border-zinc-800">
                            <div
                                class="h-28 rounded flex items-center justify-center mb-5 border border-zinc-800 relative overflow-hidden bg-gradient-to-tr from-zinc-900 to-black">
                            ...
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <span
                                    class="bg-violet-900/30 text-violet-400 text-[10px] font-bold px-2 py-0.5 rounded border border-violet-500/20">ANNUAL</span>
                            </div>

                            <div
                                class="mt-6 pt-4 border-t border-dashed border-zinc-800 flex items-end justify-between">
                                <span class="text-zinc-400 text-xs font-medium">Total Due</span>
                                <span id="live-price-preview"
        ...
        <div class="flex-1">
            ...
            <div id="step-1" class="wizard-step bg-[#252526] border border-zinc-800 rounded-xl p-6 shadow-lg">
                ...
                <p class="text-zinc-500 text-sm mb-6">Select your primary operating currency and supported alternatives.
                </p>

                <div class="space-y-6">
                    <div>
                        <label class="block text-zinc-400 text-xs font-bold uppercase tracking-wider mb-2">Primary
                            Currency</label>
                        <select name="currency[default_currency]" id="input-default-currency"
                            class="w-full bg-[#252526]  border border-zinc-700 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 transition-colors">
                    ...
                    <div style="display: none;;">
                        <label class="block text-zinc-400 text-xs font-bold uppercase tracking-wider mb-2">Supported
                            Currencies</label>
                        <div
                            class="grid grid-cols-2 md:grid-cols-3 gap-2 h-64 overflow-y-auto stealth-scroll pr-2 border border-zinc-800 p-2 rounded-lg bg-[#252526] ">
                            <?php foreach ($currencies as $code => $c): ?>
                                <label
                                    class="flex items-center gap-3 bg-[#252526] border border-zinc-800 rounded-md px-3 py-2 cursor-pointer hover:border-violet-500/50 transition-colors group">
                                    <input type="checkbox" name="accepted_currencies[]" value="<?= $code ?>"
                                        class="w-4 h-4 bg-[#252526]  border-zinc-700 accent-violet-600 rounded"
            ...
            <div id="step-2" class="wizard-step hidden bg-[#252526] border border-zinc-800 rounded-xl p-6 shadow-lg">
                ...
                <p class="text-zinc-500 text-sm mb-6">Customize how price tags are displayed across your SaaS
                    application.</p>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-zinc-400 text-xs font-bold uppercase tracking-wider mb-2">Symbol
                            Position</label>
                        <select name="currency[symbol_position]" id="input-symbol-pos"
                            ...
                            <option value="right" <?= $cur_symbol_pos === 'right' ? 'selected' : '' ?>>Right (100$)
                            </option>
                            <option value="left_space" <?= $cur_symbol_pos === 'left_space' ? 'selected' : '' ?>>Left +
                                Space ($ 100)</option>
                            <option value="right_space" <?= $cur_symbol_pos === 'right_space' ? 'selected' : '' ?>>Right +
                                Space (100 $)</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-zinc-400 text-xs font-bold uppercase tracking-wider mb-2">Decimal
                            Places</label>
                    ...
                    <div>
                        <label class="block text-zinc-400 text-xs font-bold uppercase tracking-wider mb-2">Thousand
                            Separator</label>
                        <select name="currency[thousand_separator]" id="input-thou-sep"
                            ...
                            <option value="space" <?= $cur_thousands === 'space' ? 'selected' : '' ?>>Space (1 234.56)
                            </option>
            ...
            <div id="step-3" class="wizard-step hidden bg-[#252526] border border-zinc-800 rounded-xl p-6 shadow-lg">
                ...
                <p class="text-zinc-500 text-sm mb-6">Finalize how foreign transactions are managed on your platform.
                </p>

                <div class="bg-[#252526]  border border-zinc-800 rounded-lg p-5 mb-8">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-zinc-500 text-xs mt-1">Automatically update pricing daily using open market
                                APIs.</p>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="currency[auto_conversion]" value="1" class="sr-only peer"
                                <?= $cur_auto_convert === '1' ? 'checked' : '' ?>>
                            <div
                                class="w-11 h-6 bg-zinc-800 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-zinc-600 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-violet-600">
                            </div>
                        </label>
                    </div>
                </div>
