...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_product') {
        ...
        $resolveCategory = function (string $catName) use ($db, &$categoryCache) {
            ...
            if (array_key_exists($key, $categoryCache))
...
<div class="flex flex-1 flex-col overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto overflow-x-hidden bg-[#1b1b1c] p-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div class="flex items-center gap-2">
                <button onclick="openImportModal()"
                    class="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2.5 rounded-lg flex items-center gap-2 transition-all duration-200 font-medium text-sm border border-white/5">
                    <i class="bi bi-cloud-upload"></i> Import from Shopify
                </button>
                <button onclick="openModal('add')"
                    class="bg-purple-600 hover:bg-purple-500 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 transition-all duration-200 font-medium text-sm shadow-lg shadow-purple-600/20 hover:shadow-purple-500/30">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div
                class="bg-gray-800 rounded-xl border border-white/5 p-5 transition-all duration-200 hover:border-white/10">
            ...
            <div
                ...
                <p class="text-2xl font-bold text-white">
                    <?php echo __CURRENCY_SIGN__ . number_format($totalValue, 2); ?>
                </p>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 mb-6">
            <div class="relative flex-1">
                ...
                <input type="text" id="searchInput" placeholder="Search products..."
                    class="w-full bg-gray-800 border border-white/5 rounded-lg pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors">
            </div>
            <select id="categoryFilter"
                class="bg-gray-800 border border-white/5 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors min-w-[180px] appearance-none cursor-pointer"
                style="background-image:url('data:image/svg+xml;utf8,<svg fill=\" %239ca3af\" viewBox=\"0 0 20 20\"
                ...
                </svg>');background-repeat:no-repeat;background-position:right 12px
                center;background-size:16px;padding-right:36px;">
            ...
            <select id="stockFilter"
                class="bg-gray-800 border border-white/5 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors min-w-[150px] appearance-none cursor-pointer"
                style="background-image:url('data:image/svg+xml;utf8,<svg fill=\" %239ca3af\" viewBox=\"0 0 20 20\"
                xmlns=\"http://www.w3.org/2000/svg\">
                <path fill-rule=\"evenodd\" d=\"M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08
                    1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z\" clip-rule=\"evenodd\" />
        ...
        <div class="rounded-xl bg-gray-800 border border-white/5 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-400">
                    <thead
                        class="bg-gray-800/80 text-[11px] uppercase text-gray-500 tracking-wider border-b border-white/5">
                        <tr>
                    ...
                    <tbody id="productsTableBody" class="divide-y divide-white/5">
                            ...
                            <tr id="emptyState">
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        <div
                                            class="h-20 w-20 rounded-2xl bg-gray-700/50 flex items-center justify-center mb-4">
                                            <i class="bi bi-box-seam text-4xl text-gray-600"></i>
                                        </div>
                                        <p class="text-gray-400 font-medium mb-1">No products yet</p>
                                        <p class="text-gray-600 text-sm mb-4">Get started by adding your first product to
                                            the store.</p>
                                        <button onclick="openModal('add')"
                                            class="bg-purple-600 hover:bg-purple-500 text-white text-sm px-4 py-2 rounded-lg flex items-center gap-2 transition-all duration-200">
                                            <i class="bi bi-plus-lg"></i> Add Product
                                        </button>
                                    </div>
                                </td>
                        ...
                        <?php else: ?>
                            <?php foreach ($products as $product):
                                ...
                                ?>
                                <tr class="product-row hover:bg-white/[0.02] transition-colors duration-150"
                                        ...
                                        <div class="flex items-center gap-3.5">
                                            <?php if (!empty($product['image'])): ?>
                                                <img src="<?php echo htmlspecialchars($product['image']); ?>" alt=""
                                                    class="h-11 w-11 rounded-lg object-cover bg-gray-700 ring-1 ring-white/10 flex-shrink-0">
                                            <?php else: ?>
                                                <div
                                                    class="h-11 w-11 rounded-lg bg-gray-700/60 flex items-center justify-center text-gray-500 ring-1 ring-white/5 flex-shrink-0">
                                    ...
                                    <td class="px-6 py-4">
                                        <?php if (!empty($product['category_name'])): ?>
                                            <span
                                                class="inline-flex items-center gap-1.5 rounded-md bg-gray-700/50 px-2.5 py-1 text-xs font-medium text-gray-300 ring-1 ring-white/5">
                                    ...
                                    <td class="px-6 py-4 text-white font-medium tabular-nums">
                                        <?php echo __CURRENCY_SIGN__ . number_format($product['price'], 2); ?>
                                    </td>
                                    <td class="px-6 py-4 text-white tabular-nums"><?php echo $product['stock']; ?></td>
                                    <td class="px-6 py-4">
                                        <span
                                            class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold <?php echo $badgeClass; ?>">
                                            <?php echo $badgeText; ?>
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex gap-1 justify-end">
                                            ...
                                            <form method="POST"
                                                ...
                                                <button type="submit"
                                                    class="h-8 w-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-all duration-150"
                                                    title="Delete">
...
<div id="productModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-start sm:items-center justify-center min-h-[100dvh] px-3 py-3 sm:px-4 sm:py-6">
        ...
        <div id="modalBackdrop"
            class="fixed inset-0 bg-[#252526] /60 backdrop-blur-sm transition-opacity duration-300 opacity-0"
            onclick="closeModal()"></div>

        <!-- Modal Panel -->
        <div id="modalPanel"
            class="relative w-full max-w-3xl bg-gray-800 rounded-t-3xl sm:rounded-2xl shadow-2xl shadow-black/40 border border-white/10 transform transition-all duration-300 scale-95 opacity-0 max-h-[calc(100dvh-1.5rem)] flex flex-col overflow-hidden">
            <form method="POST" id="productForm" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                ...
                <div class="px-4 sm:px-6 py-4 sm:py-5 space-y-5 overflow-y-auto flex-1 min-h-0">
                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1.5">Product Name <span
                                class="text-red-400">*</span></label>
                        <input type="text" name="name" id="productName" required
                            class="w-full bg-gray-700 border border-white/5 rounded-lg px-3.5 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors placeholder-gray-500"
                    ...
                    <div>
                        ...
                        <select name="category_id" id="productCategory"
                            class="w-full bg-gray-700 border border-white/5 rounded-lg px-3.5 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors appearance-none cursor-pointer"
                            style="background-image:url('data:image/svg+xml;utf8,<svg fill=\" %239ca3af\" viewBox=\"0 0
                            ...
                            <path fill-rule=\"evenodd\" d=\"M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0
                                ...
                                clip-rule=\"evenodd\" /></svg>');background-repeat:no-repeat;background-position:right
                            12px center;background-size:16px;padding-right:36px;">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="productDescription" rows="3"
                            class="w-full bg-gray-700 border border-white/5 rounded-lg px-3.5 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors placeholder-gray-500 resize-none"
                            placeholder="Brief product description..."></textarea>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1.5">Price
                                (<?php echo __CURRENCY_SIGN__; ?>) <span class="text-red-400">*</span></label>
                            <input type="number" step="0.01" min="0" name="price" id="productPrice" required
                                class="w-full bg-gray-700 border border-white/5 rounded-lg px-3.5 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors placeholder-gray-500"
                                placeholder="0.00">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1.5">Stock Quantity <span
                                    class="text-red-400">*</span></label>
                            <input type="number" min="0" name="stock" id="productStock" required
                                class="w-full bg-gray-700 border border-white/5 rounded-lg px-3.5 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors placeholder-gray-500"
                                placeholder="0">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div class="space-y-3">
                            ...
                            <div id="imagePreviewContainer" class="hidden">
                                <div
                                    class="relative inline-block rounded-xl overflow-hidden bg-[#1b1b1c] border border-white/10 group">
                                    <img id="imagePreview" src="" class="h-28 w-28 object-cover">
                                    <button type="button" onclick="removeImage()"
                                        class="absolute inset-0 bg-[#252526] /50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                            ...
                            <label class="relative block cursor-pointer">
                                <div
                                    class="flex items-center justify-center gap-3 bg-gray-700/40 border border-dashed border-white/10 rounded-xl px-4 py-5 text-gray-400 hover:border-purple-500/50 hover:text-gray-300 hover:bg-gray-700/60 transition-all duration-200">
                                ...
                                <input type="file" name="product_image_file" id="productImageFile" accept="image/*"
                            ...
                            <div class="relative">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <span class="text-[10px] text-gray-600 font-semibold uppercase tracking-widest">or
                                        paste URL</span>
                                    <div class="h-px flex-1 bg-white/5"></div>
                                </div>
                                <input type="text" name="image" id="productImage"
                                    class="w-full bg-gray-700 border border-white/5 rounded-lg px-3.5 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors placeholder-gray-500"
                                    placeholder="https://example.com/image.jpg" oninput="previewUrlImage(this.value)">
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="file" name="gallery_image_files[]" id="productGalleryFiles" accept="image/*"
                                multiple class="hidden" onchange="handleGalleryUpload(this)">
                            <div class="rounded-xl border border-dashed border-white/10 bg-gray-700/30 p-4 space-y-4">
                                <div class="flex items-center justify-between gap-3">
                                    <div>
                                        ...
                                        <p class="text-[11px] text-gray-500 mt-0.5">Select multiple files and we’ll
                                            store them as the product gallery.</p>
                                    </div>
                                    <button type="button"
                                        onclick="document.getElementById('productGalleryFiles').click()"
                                        class="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs font-medium text-white transition-colors hover:border-purple-500/40 hover:bg-purple-500/10">
                                        <i class="bi bi-folder-plus"></i> Choose files
                                    </button>
                                </div>
                                <div id="galleryPreviewEmpty"
                                    class="rounded-lg border border-white/5 bg-gray-800/40 px-4 py-5 text-center text-xs text-gray-500">
                                    No gallery images selected yet.
                                </div>
                                <div id="galleryPreviewGrid" class="grid grid-cols-2 gap-3"></div>
                            </div>
                            <p class="text-[11px] text-gray-500 leading-relaxed">The main image stays as the primary
                                image. Gallery images are saved in order and used for product slides.</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p class="text-[11px] text-gray-500 mt-0.5">Add size, color, or any other option. Each
                                    row can have its own image.</p>
                            </div>
                ...
                <!-- Modal Footer -->
                <div
                    class="px-4 sm:px-6 py-4 border-t border-white/5 flex flex-col-reverse sm:flex-row sm:justify-end gap-2 bg-gray-800/95 backdrop-blur">
                    <button type="button" onclick="closeModal()"
                        class="px-4 py-2.5 rounded-lg border border-white/10 text-sm font-medium text-gray-300 hover:text-white hover:bg-white/5 transition-all duration-150">
                        Cancel
                    </button>
                    <button type="submit"
                        class="px-5 py-2.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-sm font-medium text-white shadow-lg shadow-purple-600/20 hover:shadow-purple-500/30 transition-all duration-200">
...
<div id="importModal" class="fixed inset-0 z-50 hidden" aria-labelledby="import-modal-title" role="dialog"
    aria-modal="true">
    <div class="flex items-start sm:items-center justify-center min-h-[100dvh] px-3 py-3 sm:px-4 sm:py-6">
        <div id="importBackdrop"
            class="fixed inset-0 bg-[#252526] /60 backdrop-blur-sm transition-opacity duration-300 opacity-0"
        ...
        <div id="importPanel"
            ...
            <div id="importStateUpload" class="px-4 sm:px-6 py-6 sm:py-8 overflow-y-auto flex-1 min-h-0">
                <label id="importDropzone" class="relative block cursor-pointer">
                    <div
                        class="flex flex-col items-center justify-center gap-3 bg-gray-700/30 border-2 border-dashed border-white/10 rounded-xl px-6 py-12 text-gray-400 hover:border-purple-500/50 hover:text-gray-300 hover:bg-gray-700/50 transition-all duration-200">
                ...
                <div id="importUploadError"
                    class="hidden mt-3 px-4 py-3 rounded-lg bg-red-500/10 border border-red-500/20 text-sm text-red-300">
                </div>
            </div>
            ...
            <div id="importStatePreview" class="hidden flex-1 flex flex-col min-h-0">
                <div class="px-4 sm:px-6 pt-4 pb-3 border-b border-white/5">
                    <div class="flex flex-wrap gap-2" id="importSummary">
                        <span
                            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-green-500/10 text-green-400 ring-1 ring-green-500/20 text-xs font-medium">
                            <i class="bi bi-plus-circle"></i> <span id="importSummaryInsert">0</span> to insert
                        </span>
                        <span
                            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-blue-500/10 text-blue-400 ring-1 ring-blue-500/20 text-xs font-medium">
                            <i class="bi bi-arrow-clockwise"></i> <span id="importSummaryUpdate">0</span> to update
                        </span>
                        <span
                            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-gray-500/10 text-gray-400 ring-1 ring-gray-500/20 text-xs font-medium">
            ...
            <div id="importStateResult" class="hidden px-4 sm:px-6 py-10 text-center overflow-y-auto flex-1 min-h-0">
                ...
                <div id="importResultErrors"
                    class="hidden mt-4 text-left bg-[#1b1b1c]/50 border border-white/5 rounded-lg p-3 max-h-40 overflow-y-auto">
            ...
            <div
                class="px-4 sm:px-6 py-4 border-t border-white/5 flex flex-col-reverse sm:flex-row sm:justify-end gap-2 bg-gray-800/95 backdrop-blur">
                <button type="button" id="importBackBtn" onclick="importGoToUpload()"
                    class="hidden px-4 py-2.5 rounded-lg border border-white/10 text-sm font-medium text-gray-300 hover:text-white hover:bg-white/5 transition-all duration-150">
                    Back
                </button>
                <button type="button" id="importCancelBtn" onclick="closeImportModal()"
                    class="px-4 py-2.5 rounded-lg border border-white/10 text-sm font-medium text-gray-300 hover:text-white hover:bg-white/5 transition-all duration-150">
                    Cancel
                </button>
                <button type="button" id="importConfirmBtn" onclick="importConfirm()"
                    class="hidden px-5 py-2.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-sm font-medium text-white shadow-lg shadow-purple-600/20 hover:shadow-purple-500/30 transition-all duration-200">
                    <span id="importConfirmLabel">Confirm Import</span>
                </button>
                <button type="button" id="importDoneBtn" onclick="importFinish()"
                    class="hidden px-5 py-2.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-sm font-medium text-white shadow-lg shadow-purple-600/20 hover:shadow-purple-500/30 transition-all duration-200">
                    Done
                </button>
            </div>
...
<script>
    ...
    function renderGalleryPreviews() {
        ...
        galleryImages.forEach((item, index) => {
            ...
            card.className = 'group relative overflow-hidden rounded-xl border border-white/10 bg-[#1b1b1c]/60';
            card.innerHTML = `
                    ...
                    <div class="absolute inset-x-0 bottom-0 flex items-end justify-between gap-2 bg-gradient-to-t from-black/80 via-black/25 to-transparent p-3">
                        ...
                        <button type="button" onclick="removeGalleryImage(${index})" class="rounded-md bg-[#252526] /45 px-2 py-1 text-[11px] font-medium text-white opacity-0 transition-opacity group-hover:opacity-100 hover:bg-red-500/70">
                            Remove
                        </button>
                    </div>
                `;
